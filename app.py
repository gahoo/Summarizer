...
def save_cookies(request):
    """Decodes a Netscape-formatted cookie string from the 'yt-cookies' header and saves it to a temporary file."""
    encoded_cookies = request.headers.get('yt-cookies')
    if not encoded_cookies:
        return None

    try:
        # The client now sends a perfectly formatted Netscape string, Base64 encoded.
        decoded_cookies = base64.b64decode(encoded_cookies).decode('utf-8')
        
        # Create a temporary file and write the decoded content directly.
        with tempfile.NamedTemporaryFile(mode='w+', delete=False, suffix='.txt', encoding='utf-8') as temp_file:
            temp_file.write(decoded_cookies)
